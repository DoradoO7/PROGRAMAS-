...
usuarioo = int.Parse(args[0]);
if(usuarioo == 1){usuario = "Alumno"; subtot += 100;}
else if(usuarioo == 2){usuario = "Trabajador"; subtot += 200;}
else {usuario = "Docente"; subtot += 500;}
paq = int.Parse(args[1]);
if(paq == 1){paquete = "Conferencias"; subtot += 600;}
else if(paq == 2){paquete = "Eventos"; subtot += 800;}
else {paquete = "Todo"; subtot += 900;}
cantidad = int.Parse(args[2]);
subtot = subtot * cantidad;

if(subtot >= 5000) {
    if(usuarioo == 1)desc = 0.20f;
    else if(usuarioo == 2)desc = 0.10f;
    else desc = 0.05f;
}
total = subtot - (subtot * desc);
Console.WriteLine($"**Universidad Patito ------------------------------------------------------------");
Console.WriteLine($"**Tu pedido fue: {cantidad} / Tipo de Usuario : {usuario} / Tipo de Paquete: {paquete}");
